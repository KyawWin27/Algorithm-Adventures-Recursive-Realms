...
void ArrayBag<ItemType>::operator/=(const ArrayBag<ItemType>& Array_bag){
    for(int i = 0; i < getCurrentSize(); i++){   //loops through the size of the bag
        if((getCurrentSize() == DEFAULT_CAPACITY)){     
            break;
        }
        if(!contains(Array_bag.items_[i])){     // combines the contents from both ArrayBag objects but excluding duplicates
            add(Array_bag.items_[i]);    // adding items from the argument bag
...
void ArrayBag<ItemType>::operator += (const ArrayBag<ItemType>& Array_Bag){
    for(int i = 0; i < getCurrentSize(); i++){      //loops through the size of the bag
        ...
        add(Array_Bag.items_[i]);      // adding items from the argument bag as long as there is space
